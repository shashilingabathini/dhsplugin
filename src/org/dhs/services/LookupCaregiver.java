...
public class LookupCaregiver  extends PluginService {
	...
	public void execute(PluginServiceCallbacks callbacks,
			...
			throws Exception {
		String methodName = "execute - LookupCaregiver  - service";
		...
		try {
			...
			if(isLocal) {
				...
				logger.logInfo(LookupCaregiver.class,methodName,body.toString());
				System.out.println(body);
				String caregiverConfigURL = (String)((JSONObject)util.getConfigurationKeys(callbacks)).get("CaregiverURL");
				logger.logInfo(LookupCaregiver.class, methodName, caregiverConfigURL);
				System.out.println("caregiverConfigURL :"+caregiverConfigURL);
				logger.logInfo(LookupCaregiver.class,methodName,caregiverConfigURL);
				if(caregiverConfigURL == null) {
					caregiverConfigURL =  Configuration.getConfigProp("org.dhs.api.endpoint.caregiver.url");
				}
				json = util.executeAPIArray(caregiverConfigURL ,"POST",body,false);
				logger.logInfo(LookupCaregiver.class,methodName,json.toString());
			}
			...
			jsonResults.put("errorCode", "ERR_GET_CAREGIVER_DETAILS");
		}
